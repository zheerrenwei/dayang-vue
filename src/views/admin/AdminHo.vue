<template>
  <AdminNavbar>
    <div style="margin-bottom: 20px">
      ...
      <el-input placeholder="请输酒店名称" style="width: 15%" v-model="page.name" />
      <el-button type="primary" style="margin-left: 20px" @click="pageQuery">查询</el-button>
      <el-button type="primary" style="float: right" @click="showDialog = true">+添加酒店</el-button>
    </div>
    ...
    <el-dialog v-model="showDialog" title="添加酒店" width="50%" @close="handleClose">
      <el-form ref="formRef" :model="formData" :rules="formRules" label-width="120px" label-position="right">
        ...
        <el-form-item label="酒店星级" prop="openTime">
          <el-input v-model="formData.stars" placeholder="酒店星级" clearable />
        </el-form-item>
    ...
    <el-dialog v-model="showDialog2" title="编辑酒店" width="50%" @close="handleClose">
      <el-form ref="formRef" :model="formData2" :rules="formRules" label-width="120px" label-position="right">
        <el-form-item label="酒店名称" prop="name">
          <el-input v-model="formData2.name" placeholder="请输入酒店名称" clearable />
        </el-form-item>
        <el-form-item label="酒店描述" prop="description">
        ...
        <el-form-item label="酒店星级" prop="openTime">
          <el-input v-model="formData2.stars" placeholder="酒店星级" clearable />
        </el-form-item>
        <el-form-item label="图片" prop="image">
          <el-upload method="post" list-type="picture-card" :limit="1" :auto-upload="false"
...
const getData = async () => {
  try {
    const response = await axiosInstance.get('/admin/hotels/page', {
      headers: {
...
const handleDelete = async (hotelId) => {
  try {
    ...
    const response = await axiosInstance.delete(`/admin/hotels/${hotelId}`);
    if (response.data.code === 1) {
...
const handleSubmit = async () => {
  try {
    ...
    const response = await axiosInstance.post('/admin/hotels', formData);
    if (response.data.code === 1) {
