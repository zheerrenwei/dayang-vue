<template>
  <AdminNavbar>
    ...
    <el-table :data="tableData" border style="width: 100%;margin-bottom:20px;height:420px">
      ...
      <el-table-column prop="ticketName" label="门票名" width="200" />
...
const getData = async () => {
  try {
    const response = await axiosInstance.get('/admin/ticket_reservations/page', {
      headers: {
...
const handleDelete = async (reservationId) => {
  try {
    ...
    const response = await axiosInstance.delete(`/admin/ticket_reservations/${reservationId}`);
    if (response.data.code === 1) {
...
const handleEdit = async (reservationId) => {
  try {
    const response = await axiosInstance.get(`/admin/ticket_reservations/${reservationId}`);
    if (response.data.code === 1) {
